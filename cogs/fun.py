...
class Fun(commands.Cog):
  ...
  #8BALL COMMAND
  @commands.command()
  async def eightball(self, ctx, *, user_question: str):
    reply = random.choice(eightball_replies)
    result = discord.Embed(
      title = "MAGIC 8 BALL",
      color = discord.Color.blue(),
    )
    result.add_field(name = "QUESTION", value = user_question, inline = False)
    result.add_field(name = "RESPONSE", value = reply, inline = False)
